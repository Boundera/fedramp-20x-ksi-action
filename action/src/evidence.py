...
Generates the complete evidence pack artifact for KSI-MLA-EVC.
"""
...
class EvidencePackBuilder:
    """Builder for FedRAMP KSI-MLA-EVC evidence pack."""
    ...
    def __init__(self, output_dir: str | Path = "."):
        ...
        self.evidence_dir = self.output_dir / "evidence" / "ksi-mla-evc"
    ...
    def write_json_file(
        ...
        if subdir:
            ...
            rel_path = f"evidence/ksi-mla-evc/{subdir}/{filename}"
        else:
            target_dir = self.evidence_dir
            rel_path = f"evidence/ksi-mla-evc/{filename}"
    ...
    def write_hashes(self) -> None:
        ...
        self.files_written.append(
            ("evidence/ksi-mla-evc/hashes.sha256", "SHA-256 hashes of all evidence files")
        )
    ...
    def write_results_summary(
        ...
        """
        summary_text = {
            KSIStatus.PASS: "All KSI-MLA-EVC criteria passed. Terraform configuration evaluated successfully.",
            KSIStatus.FAIL: "KSI-MLA-EVC evaluation failed. Review criteria results for details.",
            KSIStatus.ERROR: "KSI-MLA-EVC evaluation encountered errors. Unable to determine compliance status.",
        }
