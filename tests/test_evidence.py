...
class TestEvidencePackBuilder:
    ...
    def test_setup_directories(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (Path(tmpdir) / "evidence" / "ksi-mla-evc").exists()
            assert (Path(tmpdir) / "evidence" / "ksi-mla-evc" / "declared").exists()
    ...
    def test_write_json_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(path) as f:
                loaded = json.load(f)
            assert loaded == data
            assert ("evidence/ksi-mla-evc/test.json", "Test file") in builder.files_written
...
class TestBuildEvidencePack:
    ...
    def test_builds_complete_evidence_pack(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert artifact_name.startswith("evidence_ksi-mla-evc_")
            ...
            with open(results_path) as f:
                results = json.load(f)
            assert results["ksi_id"] == "KSI-MLA-EVC"
